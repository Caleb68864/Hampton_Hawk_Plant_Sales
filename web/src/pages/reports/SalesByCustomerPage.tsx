...
import type { SalesByCustomerRow } from '@/types/reports.js';
import { exportToCsv } from '@/utils/csvExport.js';

type SortKey = keyof SalesByCustomerRow;
type SortDir = 'asc' | 'desc';

const COLUMNS: Array<{ key: SortKey; label: string; align?: 'left' | 'right' }> = [
  { key: 'customerDisplayName', label: 'Customer', align: 'left' },
...
export function SalesByCustomerPage() {
  ...
  const { data, loading, error } = useAsyncData(() => reportsApi.salesByCustomer(), '', 'Failed to load sales by customer report.');
  const rows: SalesByCustomerRow[] = useMemo(() => data ?? [], [data]);
  ...
  function handleDownloadCsv() {
    ...
    const filename = `sales-by-customer-${new Date().toISOString().slice(0, 10)}.csv`;
      ...
      <section className="rounded-lg border border-gray-200 bg-white p-6">
        {sortedRows.length === 0 ? (
          <BotanicalEmptyState
            ...
            description="Once orders are recorded, totals by customer will appear here."
          />
        ) : (
              ...
              <tbody className="divide-y divide-gray-100">
                {sortedRows.map((row) => (
                  <tr key={row.customerId}>
                    <td className="px-3 py-2 text-gray-900">{row.customerDisplayName}</td>
...
function sortRows(rows: SalesByCustomerRow[], key: SortKey, dir: SortDir): SalesByCustomerRow[] {
